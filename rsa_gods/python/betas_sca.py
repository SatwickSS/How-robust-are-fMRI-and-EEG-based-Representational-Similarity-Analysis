"""
Perform Beta Weight Estimation using multiple pipeline cofigurations
...
class SingleTrialBetas(GODGLM):
    ...
    def __init__(
            ...
            rescale_runwise: str = 'off',
            rescale_global: str = 'off',
        ...
        self.hrflib=np.load('/scratch/satwick22/DATA1/satwick22/Documents/fMRI/code_CDIS/srcpy/hrf.npy')
        ...
        assert rescale_runwise in ['z', 'psc', 'center','off']  # don't allow uncentered data
        self.rescale_runwise_data = rescale_runwise
        assert rescale_global in ['z', 'psc', 're-center', 'off']
        self.rescale_global = rescale_global
        ...
        assert hrf_method in ['mean','mode']
        self.hrf_method = hrf_method
        ...
        self.workdirbase='/scratch/satwick22/DATA1/satwick22/Documents/fMRI/wdir/python_work_dir'
        self.workdir = pjoin(self.workdirbase, 'betas_py')
        self.outdirbase='/scratch/satwick22/DATA1/satwick22/Documents/fMRI/multimodal_concepts/generic_object_decoding_bids'
    ...
    def _make_design_df(self, event_file):
        ...
        #use the resting state as an regressor -> update nconds by 1 wherever it is used then
...
#        designmats=debug_dfs()
    ...
    def overfit_hrf(self, data, convolved_designs, noise_mats=[], chunksize_runs=1):
        ...
        if self.hrf_method=='mean':
            best_hrf_inds = np.argmax(mean_r2s, axis=0)
        elif self.hrf_method=='mode':
            runwise_besthrf=np.stack([np.argmax(chunk,axis=0)[np.newaxis,:] for chunk in chunk_scores],axis=0)
            best_hrf_inds=np.squeeze(mode(runwise_besthrf,axis=0)[0])
    ...
    def save_result(self):
        results_dir=pjoin(self.outdir,'TYPED_FITHRF_GLMDENOISE_RR.npy')
    ...
    def run(self):
        ...
        if self.batched_loading:
            ...
            #running the data creator from bold signal in batches
            batch_size=20
            data = np.vstack([self.vstack_data_masked(bold_files[batch_i-batch_size:batch_i], rescale_runwise=self.rescale_runwise_data,rescale_global=self.rescale_global)
                                         for batch_i in [i for i in range(batch_size,len(bold_files)+1,batch_size)]])
        else:
            data=self.vstack_data_masked(bold_files, rescale_runwise=self.rescale_runwise_data,rescale_global=self.rescale_global)
...
def load_betas(
        sub: str,
        glm_obj : GODGLM,
        bidsroot: str = '/scratch/satwick22/DATA1/satwick22/Documents/fMRI/multimodal_concepts/generic_object_decoding_bids',
...
def posthoc_scaling(
        sub='01',
        bidsroot='/scratch/satwick22/DATA1/satwick22/Documents/fMRI/multimodal_concepts/generic_object_decoding_bids',
...
if __name__=='__main__':
    ...
    # stb_obj.run_glm_single(run_glm_single=True)
    # stb_obj.save_result()
    #-------------------
    for rescale_runwise in ['z','psc',"center","off"]:
       for rescale_global in ['z', 'psc', 're-center', 'off']:
           for hrf_method in ['mean','mode']:  
               betas_dir=f'betas_sca_test/rescale_runwise-{rescale_runwise}/rescale_global-{rescale_global}/hrf-{hrf_method}'              
               #for sub_id in range(3,4):
               #    sub=f'0{sub_id}'
                   #sub = '05'
               bidsroot='/scratch/satwick22/DATA1/satwick22/Documents/fMRI/multimodal_concepts/generic_object_decoding_bids'
               stb_unreg = SingleTrialBetas(bidsroot=bidsroot, subject=sub, out_deriv_name=f'{betas_dir}/unregularized',cv_scheme='unregularized'
                                            ,rescale_runwise=rescale_runwise,rescale_global=rescale_global,hrf_method=hrf_method)
               stb_unreg.run()
               del stb_unreg
               gc.collect()
               stb_reg = SingleTrialBetas(bidsroot=bidsroot, subject=sub, out_deriv_name=f'{betas_dir}/regularized',
                                          rescale_runwise=rescale_runwise,rescale_global=rescale_global,hrf_method=hrf_method)
               stb_reg.run()
               posthoc_scaling(sub=sub, bidsroot=bidsroot, outdirbase=f'{betas_dir}',out_derivname='scalematched')