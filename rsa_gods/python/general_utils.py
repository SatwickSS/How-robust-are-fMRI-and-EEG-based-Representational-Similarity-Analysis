...
def calculate_upper_noise_ceilings(all_rdms,corr_func=pearsonr):
    total_nums=len(all_rdms)

    all_corrs=[]
    avg_rdms=np.mean(np.stack([all_rdms[x-1] for x in range(1,total_nums+1)]),axis=0)
    for each_sub in range(1,total_nums+1):
        sub_rdm=all_rdms[each_sub-1]
        #assert len(other_rdms)==(total_nums-1)

        all_corrs.append(np.array(corr_func(avg_rdms,sub_rdm)[0]))
    assert len(all_corrs)==total_nums
    upper_ceiling=np.array(all_corrs)
    return upper_ceiling

def calculate_lower_noise_ceilings(all_rdms,corr_func=pearsonr):
    total_nums=len(all_rdms)
    all_corrs=[]
    for each_sub in range(1,total_nums+1):
        sub_rdm=all_rdms[each_sub-1]
        other_rdms=[all_rdms[x-1] for x in range(1,total_nums+1) if x!=each_sub]
        assert len(other_rdms)==(total_nums-1)
        avg_rdms=np.mean(np.stack(other_rdms),axis=0)

        all_corrs.append(np.array(corr_func(avg_rdms,sub_rdm)[0]))
    assert len(all_corrs)==total_nums
    lower_ceiling=np.array(all_corrs)
    return lower_ceiling