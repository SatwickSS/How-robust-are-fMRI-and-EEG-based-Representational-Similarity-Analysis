...
data_dict = {_region : {sub_idx : get_region_vectors(sub_idx,_region,atlas=args.USE_ATLAS,use_train=args.USE_TRAIN_BETAS,use_test=args.USE_TEST_BETAS)  for sub_idx in range(1,args.NUM_SUB+1)} for _region in interesting_regions_mapping.keys()}
...
def plot_noise_ceilings(save_name=None,sort_idx_dict=None):
    ...
    for region in smaller_data_dict.keys():
        ...
        for n in num_voxels:
            ...
            for subject in range(1,args.NUM_SUB+1):
...
def plot_mainfig(save_name=None,normalized_version=False):
    ...
    for ind,region in enumerate(smaller_data_dict.keys()):
        ...
        #for mind,model in enumerate(model_plot_dict.keys()):
        for mind,model in enumerate(list(zip(*args.MODELS))[0]):
            ...
            val_resnet = [corr_data_dict['resnet'][region][s][0]*factors[s-1] for s in range(1,args.NUM_SUB+1)]
...
if args.MAIN_TASK == 'select_top30_voxels':
    plot_mainfig(save_name=f"{args.SAVE_DIR}/normalized_selectedvoxels_",normalized_version=True)
    plot_mainfig(save_name=f"{args.SAVE_DIR}/unnormalized_selectedvoxels_",normalized_version=False)
    plot_fancyfig(save_name=f"{args.SAVE_DIR}/fancyfig_normalized_selectedvoxels")
