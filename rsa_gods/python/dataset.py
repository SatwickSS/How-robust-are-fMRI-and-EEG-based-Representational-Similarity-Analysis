...
class GodsMRIdataset:
    """
    Data loader for the THINGS-fMRI dataset.
    """

    def __init__(self, root_path: str, validate: bool = True,sub_id='01',ses_type='perceptionTest'):
        # path attributes
        ...
        #self.target_sessions = [ses for ses in self.layout.get(return_type='id', target='session') if ses_type in ses]# get all the sessions where the functional image was recorded of the target session
        #to extract sessions of a particular subject
        #for now we are using  a dummy subject : 1st subject
        subject_id=sub_id
        self.target_sessions = [ses for ses in self.layout.get(return_type='id', target='session',subject=subject_id) if ses_type in ses]
    ...
    def get_fmriprep_t1w(self, subject):
        return pjoin(self.root_path, 'derivatives', 'fmriprep', f'sub-{subject:02d}', 'ses-anatomy','anat',
                     f'sub-{subject:02d}_ses-anatomy_desc-preproc_T1w.nii.gz')



####debug driver
if __name__=='__main__':
    GodsMRIdataset('/DATA1/satwick22/Documents/fMRI/multimodal_concepts/generic_object_decoding_bids')    
