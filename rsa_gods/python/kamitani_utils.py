...
DESKIAN_ATLAS = "/path/to/atlas"
DESTRIEUX_ATLAS = ""
TRAIN_BETAS_DIR = ""
LANGUAGE_MODELS_REPS = "/path/to/langauge_model_reps"
VISUAL_MODELS_REPS = "/path/to/visual_model_reps"

TRAIN_BETAS_DIR = ""
TEST_BETAS_DIR = "/path/to/betas"
...
def get_region_vectors(sub_idx,region_idx,atlas='default',use_train=True,use_test=True):
    ...
    if use_test==True:
        test_beta_fnames = sorted(glob(opj(test_beta_dir,'ses-perceptionTest-avg*','*MNI305.nii.gz'),recursive=True))
...
def get_model_vectors(model_name,layer_to_use,use_test=False):
    ...
    if model_name in ['BERT','GPT2','CLIP-L']:
        ...
        vectors_path = opj(LANGUAGE_MODELS_REPS,f'{model_name}',f'{layer_to_use}','*.npy')
        #read the npy file
        model_repr=np.load(glob(vectors_path)[0],allow_pickle=True)
        #flatten the representations
        model_repr=model_repr.reshape(model_repr.shape[0],-1)
        vectorlist=np.array([])
        vector_list=np.row_stack(model_repr)
        return vector_list      
    ...
    if use_test == True:
        ...
        vectors_path = opj(VISUAL_MODELS_REPS,f'{model_name}',f'{layer_to_use}','*.npy')
        #read the npy file
        model_repr=np.load(glob(vectors_path)[0],allow_pickle=True)
        #flatten the representations
        model_repr=model_repr.reshape(model_repr.shape[0],-1)
        ...
        vectorlist = []
        for valid_betaname in valid_beta_fnames:#iterate through the lsit of valid beta names
            for f in os.listdir(TEST_BETAS_DIR):   # use beta names to ensure the ordering of the features for the RDM
                if valid_betaname in f: 
                    #for each beta file, the name of the nifti image contains a index number with/without the image filename
                    x = f.split('_') #split the file name by '_' i.e get the index number
