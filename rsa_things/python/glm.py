...
class THINGSGLM(object):
    ...
    def __init__(self,
        ...
        self.n_sessions = len(self.ds.things_sessions)#CRITICAL
    ...
    def _get_bold_files(self):
        bold_files = [
            pjoin(self.subj_prepdir, f'ses-{sesname}', 'func',
                  f'sub-{self.subject}_ses-{sesname}_task-things_run-{run_i + 1:02d}_space-T1w_desc-preproc_bold.nii.gz')
    ...
    def _get_masks(self):
        masks = [
            pjoin(self.subj_prepdir, f'ses-{sesname}', 'func',
                  f'sub-{self.subject}_ses-{sesname}_task-things_run-{run_i + 1:02d}_space-T1w_desc-brain_mask.nii.gz')
            for sesname in self.ds.things_sessions for run_i in range(10)
        ]
        for mask in masks:
    ...
    def _get_nuisance_tsvs(self):
        nuisance_tsvs = [
            pjoin(self.subj_prepdir, f'ses-{sesname}', 'func',
                  f'sub-{self.subject}_ses-{sesname}_task-things_run-{run_i + 1:02d}_desc-confounds_timeseries.tsv')
            for sesname in self.ds.things_sessions for run_i in range(10)
        ]
