...
def run_melodic_wf(
        ...
        derivname: str = 'melodic_run1',
        nprocs: int = 30, melodic_mem: float = 128.,
    ...
    """
    wf_bdir = pjoin(bidsroot, 'melodicwf_wdir_run3', f'space-{space}', f'sub-{subject}')
    derivdir = pjoin(bidsroot, 'derivatives_melodic', derivname, approach)
...
class EvalMelodic:
    ...
    def __init__(
        ...
        self.out_basedir = pjoin(bidsroot, 'derivatives_melodic', out_deriv_name)#complete path of the output directory
        ...
        self.melodic_basedir = pjoin(self.bidsroot, 'derivatives_melodic', 'melodic', 'runwise', f'space-{self.space}')#specifying and creating the path to the melodic output
        ...
        self.bidsobs = self.ds.layout.get(suffix='bold', extension='nii.gz')  # all runs as pybids objects i.e bids.layout.models.BIDSImageFile
...
def write_noise_tsvs(
    ...
    def _get_comp_ts(row, bidsroot=bidsroot):
        melodic_basedir = pjoin(bidsroot, 'derivatives', 'melodic_run_n1', 'runwise', 'space-T1w',
...
if __name__ == '__main__':
    ...
    fmriprepdir = pjoin('/DATA/satwick22/Documents/fMRI/fMRI_processing/bids', 'derivatives', 'fmriprep', f'sub-{subject}')
    run_melodic_wf(
        subject, bidsroot, fmriprepdir, approach='runwise', space='T1w',
    )
    #run evalmelodic
    evalmelodic = EvalMelodic(bidsroot, out_deriv_name='melodic_features', space='T1w', random_report_prob=.0)
    #evalmelodic.runall()
    #write noise tsvs
    write_noise_tsvs(
        outdir=pjoin('/DATA/satwick22/Documents/fMRI/fMRI_processing/thingsmri_', 'derivatives', 'noise_tsvs'),
        melodic_features_tsv=pjoin(bidsroot, 'derivatives', 'melodic_features_run_n1', 'melodic_correlations_space-T1w.tsv'),
        bidsroot=bidsroot,
        thresholds=dict(edgefrac=.225, hfc=.4),
        combine_thresholds='any',
    )