...
class SingleTrialBetas(THINGSGLM):
    ...
    def __init__(
        ...
        self.outdirbase='/DATA/satwick22/Documents/fMRI/fMRI_processing/thingsmri_/'
...
if __name__ == '__main__':
    ...
    sub, bidsroot='01','/DATA/satwick22/Documents/fMRI/fMRI_processing/thingsmri_'
    #set the variable for controlling the centering of the data
    data_centering_method='psc'
    # get an unregularized estimate of responses
    stb_unreg = SingleTrialBetas(bidsroot=bidsroot, subject=sub, cv_scheme='unregularized', out_deriv_name='betas_run-05/unregularized')
    stb_unreg.run()
    ## use cross-validation and l2 regularization
    stbf_reg = SingleTrialBetas(bidsroot=bidsroot, subject=sub, out_deriv_name='betas_run-04/regularized',rescale_runwise_data=data_centering_method)
    stbf_reg.run()
    # apply post-hoc scaling
    posthoc_scaling(sub=sub, bidsroot=bidsroot, reg_derivname='betas_run-04/regularized', unreg_derivname='betas_run-04/unregularized', out_derivname='betas_run-04/scalematched')
