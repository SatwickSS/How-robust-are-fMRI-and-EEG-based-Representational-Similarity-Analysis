function make_rdms_shuffle(subjectnr, shuf, k)
    if nargin < 3
        k = 1;
    end
    ...
    %% load data
    fn = sprintf('../../derivatives/cosmomvpa_shuffled/shuffle%02i/shuf_%02i/sub-%02i_task-rsvp_cosmomvpa_shuffle%02i.mat',shuf,subjectnr,shuf);
    fprintf('loading %s\n',fn);tic
    load(fn,'ds_shuf')
    fprintf('loading data finished in %i seconds\n',ceil(toc))
    data_test = cosmo_slice(ds_shuf,ds_shuf.sa.isteststim>0,1);
    ...
    %% save
    fprintf('Saving...');tic
    outfn = sprintf('../../derivatives/rdms_shuffle/shuf_%02i/k_%02i/sub-%02i_rdm_test_images_k%02i.mat',shuf,k,subjectnr,k);
    save(outfn,'res','-v7.3')
