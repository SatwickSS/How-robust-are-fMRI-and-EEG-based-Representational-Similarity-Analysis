function make_rdms_electrode_shuffle(subjectnr, k, shuf)
    if nargin < 2
        k = 1;
    end
    ...
    %% load data
    fn = sprintf('../../lobes_cosmomvpa_shuffled/shuffle%02i/sub-%02i_task-rsvp_cosmomvpa_lobes_shuffle%02i.mat',shuf,subjectnr,shuf);
    fprintf('loading %s\n', fn); tic
    load(fn, 'lobes');
    ...
    %% save
    fprintf('Saving...');tic
    outfn = sprintf('../../derivatives/lobes_shuffle/sub-%02i_rdm_test_images_lobes_s%02i_k%02i.mat',subjectnr,shuf,k);
    save(outfn,'res','-v7.3')
